...
public class CustomerRegistrationController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getContentType() != null && request.getContentType().indexOf("multipart/form-data") >= 0) {
			...
			/*
			 * CustomerBean forUpObj = new CustomerBean(); Customer uploadFiles
			 * = new Customer();
			 */
			String photoUpPath;
			...
			if (contentType.indexOf("multipart/form-data") >= 0) {
				...
				/*
				 * Defining the threshhold size of the file that will be kept in
				 * memory
				 */
				factory.setSizeThreshold(MEMORY_THRESHOLD);
				/*
				 * defining directory to temporary store file which are larger
				 * than the threshhold memory
				 */
				factory.setRepository(new File(relativeUploadPath + TMP));
				/* creating a file upload handler */
				ServletFileUpload upload = new ServletFileUpload(factory);
				/*
				 * defining the maximum size of the file that can be uploaded to
				 * the form
				 */
				...
				/*
				 * defining the path for the directory to upload the customer
				 * photo
				 */
